...
races4 <- races3 %>% 
  select(round:driverRef, driver, constructor, country) %>% 
  ...
  select(-driverRef, -constructor, -country) %>% 
  relocate(round, country_logo, name.x, date, driver, driver_photo, constructor_logo, fastestLapTime, fastestLapSpeed)
