...
races_boxplot <- driver_results_2022 %>% 
  mutate('constructor_logo' = paste0('https://raw.githubusercontent.com/steodose/F1/master/manufacturers/', constructor, '.png')) %>%
    mutate(logos = link_to_img(constructor_logo)) %>% 
    #mutate(constructor = fct_reorder(constructor, position)) %>%
    ggplot(aes(x = fct_reorder(constructor, position), y = position, group = constructor)) +
    geom_boxplot(color = "#2E86C1", fill="#2E86C1", alpha=0.2) +
    theme_custom() +
    theme(panel.background = element_rect(fill = "floralwhite", color = "floralwhite")) +
    labs(x = "", y = "Race Finish Position",
         title = "Distribution of Race Finishes",
